 ...
 * @author malv
 */
public class RunSimulationProhavOcean {
    ...
    public static void main(String[] args) {
        ...
        for (int i=0; i<cagePos.length; i++) {
            cagePositions.add(new double[] {outerPadding + unitSizeM[0]*((double)(cagePos[i][0]) +0.5),
                outerPadding + unitSizeM[1]*(((double)cagePos[i][1]) +0.5)});
            double[] pos = cagePositions.get(cagePositions.size()-1);
            System.out.println("Cage pos (m): "+pos[0]+" x "+pos[1]);
        ...
        if (doMPI && (rank==0)) {
            enKF = new EnsembleKF(simNamePrefix, cageDims, as.nPar, dxy, null);
        }
...
//                }
                ...
                //if ((t - Math.floor(t) < dt) && (Math.floor(t) % storeIntervalInfo == 0)) {
                    ...
                    ArrayList<CageStats> cageStats = MultiCageUtils.getCageStatsRectangular(o2, mask, cagePositions, unitSizeM, dxy,
                        HYPOXIA_THRESHOLD);
            ...
            for (int j = 0; j < fishTmp.length; j++) {
                ...
                if (Double.isNaN(fish.getN(j)))
                    System.out.println("fish.getN is NaN for j=" + j);
                if (Double.isNaN(fish.getIngested(j)))
